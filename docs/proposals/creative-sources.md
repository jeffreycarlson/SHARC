...
**Compatibility bridges as first-class deliverables** (`src/sharc-*-bridge.js`):
...
| `sharc-navigation-bridge` | Intercepts **non-IAB-spec'd web-native navigation patterns** where browsers permit JS interception: `window.open`, `<a>` clicks, `<form>` submits, `<meta http-equiv="refresh">`, plus best-effort `location.href`/`assign`/`replace`. Routes intercepted paths through `SHARC.requestNavigation()`; Chromium `Location` properties are non-configurable, so `location.*` falls to the container load-event backstop. Fills the gap for creatives that use standard web patterns rather than MRAID or SafeFrame APIs. (IAB-spec'd navigation stays in the MRAID and SafeFrame bridges; this bridge does not duplicate that surface.) |
...
```javascript
{
  ...
  placementSessionId: string,
  reason: string                   // 'unsupported_renderer_protocol_version' | 'nonce_mismatch' |
                                   // 'container_origin_mismatch' | 'render_failed' |
...
Creative-initiated web-native navigation is routed through `SHARC.requestNavigation()` where browser APIs are interceptable, with the container load-event backstop enforcing the non-interceptable cases. The renderer page imports `sharc-navigation-bridge` (a first-class bridge at `src/sharc-navigation-bridge.js`, sibling to MRAID/SafeFrame/OMID bridges) and installs it before `document.write(creativeHtml)` so its interceptors apply to all creative code.
...
- **`window.location.href` setter / `location.assign()` / `location.replace()`** — best-effort only. Chromium exposes these as non-configurable own `Location` properties, so they cannot be replaced there; the container load-event backstop terminates the post-render navigation with `RENDERER_UNAUTHORIZED_NAVIGATION`.
...
| Web-native, non-IAB-spec'd navigation (`window.open`, anchor clicks, form submits, meta refresh, best-effort `location.*`) | `sharc-navigation-bridge` (new in 0.7.0) + container backstop for non-interceptable `location.*` |
...
| `window.location.href = url` (and `.assign()` / `.replace()`) | Best-effort bridge interception where replaceable; Chromium non-configurable `Location` properties fall to container load-event monitoring |
...
A SHARC **PUC compatibility bridge** — `src/sharc-puc-bridge.js`, sibling to the existing MRAID, SafeFrame, and OMID bridges — would expose PUC's interface to creatives authored against PUC, translating PUC calls into SHARC messages. This lets PUC-authored creatives run inside a SHARC container without re-authoring. The bridge fits SHARC's existing pattern: legacy interface → bridge → SHARC protocol.
...
| DD-18 | How should SHARC handle creative click-throughs that bypass `SHARC.requestNavigation()` — anchor tags, `window.location.href`, form submits, meta refresh? | **Bridge architecture + universal backstop.** Web-native (non-IAB-spec'd) navigation patterns are handled by a dedicated `sharc-navigation-bridge` (sibling to the existing MRAID, SafeFrame, OMID bridges in `src/`). The bridge intercepts `window.open`, anchor click delegation, form submit delegation, and meta-refresh stripping, and it attempts best-effort `location.href`/`assign`/`replace` wrapping where the browser permits it. Chromium exposes `Location` properties as non-configurable, so those `location.*` paths are enforced by the universal backstop rather than routed through JS. **IAB-spec'd navigation stays in its own bridge:** MRAID navigation in `sharc-mraid-bridge`, SafeFrame navigation in `sharc-safeframe-bridge`. No overlap or duplication. **Bridge load points across variants:** Creative Markup imports the bridge in the renderer page before `document.write(creativeHtml)` (operator-controlled load point); Creative URL auto-loads the bridge via the SHARC Creative SDK at SDK init (creative-author-controlled SDK script placement, but SHARC SDK is required for either variant to function). **Universal backstop:** the container counts iframe load events; any unexpected re-navigation terminates the session with `RENDERER_UNAUTHORIZED_NAVIGATION (2118)` and fires `onSecurityEvent` type `unauthorized_navigation`. JS-bypass-resistant. |
...
- [ ] **#41** `sharc-navigation-bridge` ships in `src/` as a first-class bridge module (sibling to `sharc-mraid-bridge`, `sharc-safeframe-bridge`, `sharc-omid-bridge`)
- [ ] **#41** Navigation bridge intercepts `window.open()`: injects `noopener,noreferrer`; routes URL through `SHARC.requestNavigation()`
- [ ] **#69** Browser harness verifies `window.location.href`, `location.assign()`, and `location.replace()` hit the container backstop in Chromium where `Location` is non-configurable
...
- [ ] **#41** Reference renderer supports the current `rendererProtocolVersion` and rejects unsupported versions via `SHARC:Renderer:failed { reason: 'unsupported_renderer_protocol_version' }`
