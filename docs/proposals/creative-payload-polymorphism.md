...
| `allowStorageAccessByUserActivation` | `boolean` | No | Default `true`. When `true`, the iframe sandbox includes the `allow-storage-access-by-user-activation` token, permitting creatives to invoke the Storage Access API (`document.requestStorageAccess()`) on user gesture for cross-site cookie/storage access. When `false`, the token is omitted and SAA calls fail. Default `true` matches the post-third-party-cookie measurement path that IAB-aligned measurement vendors are migrating to. See DD-22. |
| `allowModals` | `boolean` | No | Default `true`. When `true`, the iframe sandbox includes the `allow-modals` token, permitting creatives to invoke `window.alert()`, `window.confirm()`, `window.prompt()`, `window.print()`, and receive `beforeunload` events. When `false`, the token is omitted and these calls silently fail. Default `true` matches SafeFrame parity and supports legitimate use cases (age gates, subscription confirmations, "leaving page?" prompts). See DD-23. |
| `allowDownloads` | `boolean` | No | Default `true`. When `true`, the iframe sandbox includes the `allow-downloads` token, permitting creatives to trigger downloads via `<a download>` and `Content-Disposition: attachment` responses. When `false`, the token is omitted and download attempts are blocked at the browser level. Default `true` matches SafeFrame parity and supports legitimate use cases (ad-served PDFs / white papers, calendar invites `.ics`, vCards `.vcf`, coupon downloads). See DD-25. |
...
**Full sandbox (default):** `allow-scripts allow-same-origin allow-forms allow-popups allow-popups-to-escape-sandbox allow-top-navigation-by-user-activation allow-storage-access-by-user-activation allow-modals allow-downloads`
...
| `allow-modals` | `allowModals` | Conditional | See DD-23 |
| `allow-downloads` | `allowDownloads` | Conditional | See DD-25 |
...
| DD-21 | Should `allow-popups-to-escape-sandbox` be exposed as its own constructor option, or bound to `allowPopups`? | **Bound to `allowPopups`; not a separate option.** Without `allow-popups-to-escape-sandbox`, popups opened from the renderer iframe inherit SHARC's full sandbox token set — landing pages would degrade (no first-party cookies, no autoplay, restricted storage). SafeFrame includes this token by default for exactly this reason ([Chromium sample](https://googlechrome.github.io/samples/allow-popups-to-escape-sandbox/), [Mozilla bug 1190641](https://bugzilla.mozilla.org/show_bug.cgi?id=1190641) — both explicitly position the token as needed for ad iframes). The threat that omitting the token would mitigate (reverse-tabnabbing via `window.opener`) is **already mitigated** by the navigation bridge's mandatory `noopener,noreferrer` injection on all popup paths (AC: nav bridge intercepts `window.open()`). The two tokens are functionally coupled: `allow-popups-to-escape-sandbox` is meaningless without `allow-popups` (no popups to escape). There is no coherent third state ("popups allowed but inheriting sandbox") with a real production use case once the reverse-tabnab threat is closed by the nav bridge. Single-switch principle wins: `allowPopups` controls both tokens together. Adding a separate toggle would be configuration surface area without product value. |
| DD-22 | Should the renderer iframe permit the Storage Access API (`document.requestStorageAccess()`)? | **Yes — `allow-storage-access-by-user-activation` on by default; configurable via `allowStorageAccessByUserActivation: boolean`.** As third-party cookies are deprecated, the Storage Access API becomes the standard path for cross-site storage in ad iframes. Measurement vendors and identity providers are migrating to SAA as the IAB-aligned post-cookie path; default-denying it would break the very measurement coordination the proposal calls out as a release readiness item. The token requires user activation (no silent storage-access escalation), and the access prompt is browser-mediated. Configurable per the standard `allowPopups`/`allowTopNavigationByUserActivation` pattern: default permissive for SafeFrame parity and post-cookie measurement, opt-out via `false` for strict deployments where the operator does not want any cross-site storage path available to creatives. |
| DD-23 | Should the renderer iframe permit `window.alert/confirm/prompt/print` and `beforeunload`? | **Yes — `allow-modals` on by default; configurable via `allowModals: boolean`.** Legitimate use cases exist: age gates ("Are you 18+?"), subscription confirmations ("Subscribe to weekly emails?"), "leaving page?" prompts on form-submit creatives, and `window.print()` for print-coupon creatives. SafeFrame deployments include `allow-modals` in newer GAM revisions for these use cases. Threat surface is dialog-spam (creative blocks user with repeated `alert()` calls) — but modern browsers throttle this automatically: after a few sequential dialogs the browser offers a "block further dialogs from this page" UI. The residual risk is small relative to the legitimate use case loss. Configurable per the standard pattern: default permissive, opt-out for strict UX policies (e.g., publisher inventory where any dialog interrupting browsing is forbidden). |
| DD-24 | Should the renderer's Permissions Policy default-deny Privacy Sandbox APIs? | **No — `browsing-topics`, `attribution-reporting`, `private-state-token-issuance`, `private-state-token-redemption`, and `shared-storage` are NOT in the deny list.** The original proposal default-denied these, conflating "Privacy Sandbox APIs" with "sensitive APIs requiring user permission." These are categorically different: sensor/hardware/payment APIs prompt the user; Privacy Sandbox APIs are the IAB-aligned post-cookie infrastructure for measurement, attribution, and auctions. Default-denying them in SHARC would directly defeat the measurement vendor coordination the proposal explicitly calls out as a release readiness item — the Privacy Sandbox is the SDK-side API surface measurement vendors are migrating to, not a third-party-cookie-style bypass. The Permissions Policy continues to default-deny everything that prompts the user (geolocation, camera, microphone, payment, usb, serial, clipboard, sensors, idle-detection, xr-spatial-tracking, web-share, screen-wake-lock) and federated identity (`identity-credentials-get` / FedCM, which is not ad-tech relevant). Operators that don't want Privacy Sandbox APIs in their inventory can deny them at the renderer's HTTP `Permissions-Policy` response header layer — that intersects with the iframe `allow=` attribute, so the response header can be stricter than the iframe ceiling. The iframe `allow=` is the SDK-level baseline; per-deployment hardening lives one layer up. |
| DD-25 | Should the renderer iframe permit creative-initiated downloads (`<a download>`, `Content-Disposition: attachment`)? | **Yes — `allow-downloads` on by default; configurable via `allowDownloads: boolean`.** Legitimate use cases exist across direct-sold and programmatic inventory: ad-served PDFs (white papers, brochures, product catalogs), calendar invites (`.ics` files attached to event-promotion creatives), vCards (`.vcf` for contact-driven CTAs), coupon downloads, and downloadable assets in lead-gen flows. SafeFrame inventory has shipped these patterns for years. Threat surface is drive-by download (creative initiates a download without user gesture) — but modern browsers require user gesture for most download flows, antivirus / Safe Browsing scans downloaded files, and the threat is well-understood operationally. Configurable per the standard pattern: default permissive for SafeFrame parity and direct-sold use cases, opt-out via `false` for strict deployments where the operator does not want any download surface (e.g., kid-directed inventory, healthcare placements). |
...
- [ ] **#41** Permissions Policy does NOT deny Privacy Sandbox APIs (`browsing-topics`, `attribution-reporting`, `private-state-token-issuance`, `private-state-token-redemption`, `shared-storage`) — these remain available to the creative as IAB-aligned ad-tech surface (see DD-24)
...
- [ ] **#41** `allowModals: true` (default) → sandbox includes `allow-modals`
- [ ] **#41** `allowModals: false` → sandbox does NOT include `allow-modals`; creative `alert/confirm/prompt/print/beforeunload` calls silently fail
- [ ] **#41** `allowDownloads: true` (default) → sandbox includes `allow-downloads`
- [ ] **#41** `allowDownloads: false` → sandbox does NOT include `allow-downloads`; creative `<a download>` clicks and download-triggering responses are blocked by the browser
