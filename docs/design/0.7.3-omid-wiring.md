...
| `→ TERMINATED` | `AdSession.finish()` | Container closing ad |
| `placementChange` | `MediaEvents.playerStateChange(mode)` plus visibility sync | Simplified intent mapping: `expand` → `expanded`, `resize` → `normal`, `fullscreen` → `fullscreen` |

`audioVolumeChange` is not wired to OMID in 0.7.3. Mapping SHARC audio volume changes to OM SDK volume events is future work.
