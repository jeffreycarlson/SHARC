...
**Scope:** This design covers **container-side OMID wiring only**. OMID measurement is fully container-driven when both OM SDK script URLs are explicitly configured: the container loads the OM SDK, starts the session when the container reaches `READY`, fires `loaded()` and the impression on the first `ACTIVE` transition, tracks state changes, and finishes on `TERMINATE`. MRAID/SafeFrame creatives get OMID measurement without doing anything — no handshake, no creative-side API calls. Creatives can check `hasFeature('com.iabtechlab.sharc.omid')` to know OMID is available, but they do not request OMID events. If either OM SDK URL is omitted, `OmidCompatBridge` is inert for feature advertisement and does not register `com.iabtechlab.sharc.omid` in `supportedFeatures`.
...
- `OmidCompatBridge` (container-side extension) — loads OM SDK, manages session lifecycle, and contributes `com.iabtechlab.sharc.omid` to supportedFeatures only when both OM SDK script URLs are configured
...
|--------|-----------|------|------|----------|
| **Option 2 (CONFIGURED)** | `<script>` tag on publisher page, managed by container-side bridge | Works across all origins, no CORS, matches native SDK model (app owns OM SDK), zero fetch overhead | Requires publisher to provide OM SDK URLs | **✅ Adopted** |
...
The `OmidCompatBridge` extension loads OM SDK via two `<script>` injection points on the **publisher page** (NOT inside the creative iframe). Both `omSdkServiceScriptUrl` and `omSdkSessionClientUrl` are required. When either URL is omitted, the bridge does not advertise `com.iabtechlab.sharc.omid` through `getFeatureName()` / `getFeatureDescriptor()` and should not contribute to `supportedFeatures`.

```javascript
// OmidCompatBridge._ensureSdkLoaded() — called during container.load()
function _ensureSdkLoaded() {
  // 1. OM SDK Service Script (must be loaded first)
  const serviceScript = document.createElement('script');
  serviceScript.src = this.options.omSdkServiceScriptUrl;
  ...
  sessionClientScript.src = this.options.omSdkSessionClientUrl;
  sessionClientScript.async = false;
  document.head.appendChild(sessionClientScript);
}
```

**Required constructor URLs:**

| Script | Option | Required by |
|--------|-------------|-------------|
| Service Script (`omweb-v1.js`) | `omSdkServiceScriptUrl` | OM SDK Service |
| Session Client (`omid-session-client-v1.js`) | `omSdkSessionClientUrl` | Session Client API |
...
Container.load():
  5. OmidCompatBridge._ensureSdkLoaded() (if both OM SDK URLs are configured and OM SDK is not already loaded)
...
```
Container.load():
  1. OmidCompatBridge._ensureSdkLoaded()
...
| `READY` → `ACTIVE` (first active transition) | `AdEvents.impressionOccurred()` | Fired once after `loaded()` when the ad first becomes active |
...
| `TERMINATED` (after) | Session cleanup, wait 3s and leave scripts in place for publisher reuse | Post-finish cleanup |
...
Container                                    OM SDK
   ...
   |  └─ cleanup (3s delay; scripts remain loaded for publisher reuse) │
```
...
**Resolution behavior:** When `creativeMeta.apis: [7]` is passed, the bridge detection pipeline produces no bridge from that code. If no other runtime bridge is detected, the pipeline falls through to the markup scan. In the original 0.7.3 design, OMID remained an explicit extension installation decision. As of 0.7.6, the separate measurement-sidecar path can auto-install `OmidCompatBridge` when operators provide `omidAutoInstall` and bid metadata supplies `creativeMeta.measurement.omid.verificationScripts`; this still does not add an `'omid'` renderer bridge.
...
When both `omSdkServiceScriptUrl` and `omSdkSessionClientUrl` are configured, `OmidCompatBridge` contributes the following feature name string to `supportedFeatures` in `SHARC:Container:init`:

```json
"com.iabtechlab.sharc.omid"
```

No OMID version, function list, or capability descriptor is exposed to creatives through the automatic extension registration path. Creatives can only detect that the container owns OMID measurement for the session. If either OM SDK URL is omitted, `getFeatureName()` and `getFeatureDescriptor()` return `null`, so the feature is not registered.
...
| Session finish | The container finishes the OM SDK session on `close`, `destroy`, `error`, or `TERMINATED`. Maps to `AdSession.finish()` and is idempotent across repeated teardown signals. |
...
The feature is registered automatically when the `OmidCompatBridge` extension is passed to the container constructor with both required OM SDK URLs:
...
const container = new SHARCContainer({
  ...
  extensions: [
    new OmidCompatBridge({
      ...
      omSdkSessionClientUrl: 'https://cdn.example.com/omid-session-client-v1.js',
      verificationScripts: [...],
    }),
  ],
});
...
| `READY → ACTIVE` (first active transition) | `AdEvents.impressionOccurred()` | Fired once per session after `loaded()` on the first active moment |
...
- `loaded()` and impression fire on the first `ACTIVE` transition
...
interface OmidCompatBridgeOptions {
   ...
   * Required with omSdkSessionClientUrl for the bridge to advertise OMID.
   ...
   * Required with omSdkServiceScriptUrl for the bridge to advertise OMID.
   * No default is provided.
   */
...
| `omSdkServiceScriptUrl` | No construction-time validation | Required with `omSdkSessionClientUrl` for feature registration; omitted bridge is not advertised |
| `omSdkSessionClientUrl` | No construction-time validation | Required with `omSdkServiceScriptUrl` for feature registration; omitted bridge is not advertised |
...
| **4** | Implement container-side OM SDK loading (`_ensureSdkLoaded()` method on `OmidCompatBridge`) | `src/sharc-omid-bridge.js` | 1.5 hours |
...
OmidCompatBridge.prototype = {
  ...
  getFeatureName: function () {
    return this._hasSdkInjectionUrls() ? FEATURE_NAME : null;
  },
...
| Operator configures OmidCompatBridge without both OM SDK URLs | Bridge is inert for feature advertisement; no OMID feature is declared in supportedFeatures |
