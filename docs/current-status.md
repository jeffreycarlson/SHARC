...
0.7.8 is the OMID spec-compliance release. It completes the OMID bridge transition begun in 0.7.7 — the publisher-page `AdSession` from 0.7.3 stays the single source of truth, and the iframe now exposes the exact IAB OMID surface that vendor verification scripts expect. It also lands the private creative validator stack — the harness that lets us classify OMID, MRAID, and SafeFrame outcomes across real bidder corpora before 1.0.
